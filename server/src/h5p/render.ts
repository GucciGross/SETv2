...
function head(model: IEditorModel | IPlayerModel, title: string): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>${escapeHtml(title)}</title>
...
export function editorDocument(model: IEditorModel, grant: Grant, baseUrl: string): string {
  return `${head(model, 'H5P Studio')}<body><form id="studio-form"><div id="native-editor" class="h5p-editor"></div>
    ...
    (function($){
      ...
      function mount(data){editor=new H5PEditor.Editor(data&&data.library,data&&JSON.stringify(data.params),document.getElementById('native-editor'),function(){this.document.addEventListener('input',changed,true);this.document.addEventListener('change',changed,true);});status.textContent='Draft changes are private until published.';}
...
export function playerDocument(model: IPlayerModel, grant: Grant): string {
  return `${head(model, 'Interactive activity')}<body><div class="h5p-content" data-content-id="${escapeHtml(String(model.contentId))}"></div>
    <script>${bridge(grant)}
      if(window.H5P&&H5P.externalDispatcher){H5P.externalDispatcher.on('xAPI',function(event){
