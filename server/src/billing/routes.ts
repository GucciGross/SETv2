...
import { requireResourceSpace, requireSpace, requireUser } from '../lib/http.js';
import { config } from '../config.js';
import { recordActivity } from '../team/activity.js';
import { canGrantCredits } from '../lib/privileged-actions.js';
...
async function packPrice(cents: number): Promise<Stripe.Price | null> {
  ...
  if (!price) {
    ...
    price = await stripe.prices.create({ product: product.id, currency: 'usd', unit_amount: cents, lookup_key: lookupKey, nickname: `$${(cents / 100).toFixed(0)} SET Cloud credit` });
  }
...
async function balanceCents(spaceId: string): Promise<number> {
  const row = await one<{ balance: string }>(`SELECT COALESCE(SUM(amount_cents), 0)::text AS balance FROM credit_ledger WHERE space_id = $1`, [spaceId]);
...
export async function billingRoutes(app: FastifyInstance) {
  app.get('/spaces/:spaceId/billing', async (req, reply) => {
    ...
    const [balance, history, space] = await Promise.all([
      ...
      q(`SELECT id, kind, amount_cents, ref, note, created_at FROM credit_ledger WHERE space_id = $1 ORDER BY created_at DESC LIMIT 50`, [spaceId]),
      one<{ name: string }>(`SELECT name FROM spaces WHERE id = $1`, [spaceId]),
    ]);
    return { enabled: !!stripe, canGrant: canGrantCredits(req.user!.id), balanceCents: balance, packs: CREDIT_PACKS_CENTS, spaceName: space?.name ?? '', history };
  });
  ...
  app.post('/spaces/:spaceId/billing/checkout', async (req, reply) => {
    ...
    if (!(CREDIT_PACKS_CENTS as readonly number[]).includes(body.amountCents)) return reply.code(400).send({ error: `Pick one of the packs: ${CREDIT_PACKS_CENTS.join(', ')}` });
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'payment', line_items: [{ price: price.id, quantity: 1 }], client_reference_id: spaceId,
      metadata: { spaceId, packCents: String(body.amountCents) }, customer_creation: 'always',
  ...
  /** Manual grants require an operator allowlist, never workspace ownership alone. */
  app.post('/spaces/:spaceId/billing/grant', async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user) return;
    if (!canGrantCredits(user.id)) return reply.code(403).send({ error: 'Platform administrator access required' });
    const spaceId = (req.params as any).spaceId;
    if (!(await requireSpace(req, reply, spaceId, 'owner'))) return;
    const body = z.object({ amountCents: z.number().int().min(1).max(100_000_00), note: z.string().max(300).optional() }).parse(req.body);
  ...
  app.post('/billing/webhook', async (req, reply) => {
    ...
    try { event = stripe.webhooks.constructEvent((req as any).rawBody as string, sig, webhookSecret); }
    catch (e: any) { return reply.code(400).send({ error: `Signature verification failed: ${e.message}` }); }
    if (event.type === 'checkout.session.completed') {
      ...
      if (spaceId && cents > 0 && session.payment_status === 'paid') {
        ...
        const inserted = await one<{ id: string }>(
          `INSERT INTO credit_ledger (space_id, kind, amount_cents, ref, note) VALUES ($1, 'purchase', $2, $3, 'Stripe checkout') ON CONFLICT (kind, ref) DO NOTHING RETURNING id`,
          [spaceId, cents, session.id]);
        if (inserted) {
          const owner = await one<{ user_id: string }>(`SELECT user_id FROM memberships WHERE space_id = $1 AND role = 'owner' ORDER BY created_at LIMIT 1`, [spaceId]);
